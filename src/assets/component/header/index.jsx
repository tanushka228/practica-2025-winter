function HeaderComponent({ onSignUpClick, onContactClick, isAuth }) {
  let authElement;
  ...
  if (isAuth === false) {
    ...
    authElement = <span className="text-red-500">Auth error</span>;
  }
    ...
    <header className="w-full h-[108px] bg-[#171B1B] px-[130px] flex items-center">
      <div className="w-full flex items-center justify-between">
        ...
        <div className="flex items-center gap-10">
          {authElement}
          <button className="bg-[#F2B766] h-[47px] w-[183px] rounded-full text-[#171B1B] text-[16px] transition hover:bg-[#d99f4f] hover:scale-105" onClick={onContactClick} > Contact us </button>
        </div>
      </div>
    </header>
  );
